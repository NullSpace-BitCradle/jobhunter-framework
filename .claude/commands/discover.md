...
# /discover — Scan ATS platforms for new matching roles

Thin wrapper around the Python discovery tool. Runs the scan, then parses the resulting digest and shows a compact summary so the user can quickly pick what to `/apply` for.
...
```bash
cd ~/Projects/jobhunter-framework/discovery && source venv/bin/activate && python main.py $ARGUMENTS
```
